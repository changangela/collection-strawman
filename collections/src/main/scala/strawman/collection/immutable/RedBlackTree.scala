...
private[collection] object RedBlackTree {
  ...
  @tailrec
  def nth[A, B](tree: NETree[A, B], n: Int): Tree[A, B] = {
  ...
  private[this] final object NList {
    ...
    def foldLeft[A, B](xs: NList[A] | Null, z: B)(op: (B, A) => B): B = {
      var acc = z
      var these = xs
      while (these ne null) {
        acc = op(acc, these.nn.head)
        these = these.nn.tail
      }
      acc
    }
